			...
			if(data[0].equals(String.valueOf(VARIABLE))) {
				if(data.length > 2) {
					...
					if(lowerCase.equals(IMPORT)) {
						...
						try {
							final String url = importLocation;
							final URL urlObject = new URL(url);

							CurseEventHandling.forEach(handler ->
									handler.preDownloadDocument(url));

							final String string = NetworkUtils.read(urlObject);

							CurseEventHandling.forEach(handler ->
									handler.postDownloadDocument(url));
						...
						lines.addAll(i + 1, toImport.stream().filter(string -> {
							string = string.trim();
							...
							if(string.charAt(0) == VARIABLE) {
								final String[] args = StringUtils.splitWhitespace(string);
								return args[1].equalsIgnoreCase(IMPORT);
							}
